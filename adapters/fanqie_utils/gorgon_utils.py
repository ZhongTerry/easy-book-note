...
from common import utils, Httpx
...
async def gorgonRequest(url, options={'method': 'GET', 'headers': {}}):
    data_md5 = '' if (not options.get(
        'body')) else utils.createMD5(options['body'])
    if (data_md5):
        options['headers']['x-ss-stub'] = data_md5
    gorgon = _g.calculate('?'.join(url.split('?')[1:]), options['headers'])
    if ('x-ss-stub' in options['headers']):
        del options['headers']['x-ss-stub']
    for k, v in gorgon.items():
        options['headers'][k] = v
    return await Httpx.AsyncRequest(url, options)


def gorgonRequestSync(url, options={'method': 'GET', 'headers': {}}):
    data_md5 = '' if (not options.get(
        'body')) else utils.createMD5(options['body'])
    if (data_md5):
        options['headers']['x-ss-stub'] = data_md5
